...
class WakeRecoveryPolicyTest {
    private val policy = WakeRecoveryPolicy(minimumScreenOffDurationMs = 60_000L)

    @Test
    fun `ignores short screen off interval`() {
        assertNull(policy.recoveryDurationMs(screenOffAtMs = 1_000L, screenOnAtMs = 60_999L))
    }
    ...
    fun `accepts interval at threshold`() {
        assertEquals(
            60_000L,
            policy.recoveryDurationMs(screenOffAtMs = 1_000L, screenOnAtMs = 61_000L),
        )
    }
